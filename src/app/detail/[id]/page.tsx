        ...
        <div className="flex h-screen max-h-screen min-h-screen w-full justify-center bg-white ">
            <div className="h-full w-2/4 py-20">
                ...
                <div className="my-9 flex flex-row justify-between gap-2">
                    <div className="flex w-full justify-center rounded-xl border border-gray-100 bg-gray-100 p-2 text-sm text-gray-500 hover:border-gray-500"> {formattedDate} </div>
                    <div className="flex w-full justify-center rounded-xl border border-gray-100 bg-gray-100 p-2 text-sm text-gray-500 hover:border-gray-500"> {diary.weather} </div>
                    <Link href={`/emotions/${diary.emotion}`} className="flex w-full justify-center rounded-xl border border-gray-100 bg-gray-100 p-2 text-sm text-gray-500 hover:border-gray-500"> {diary.emotion} </Link>
                </div>
                <div className="h-2/3 text-base text-gray-800">{diary.content}</div>
                <div className="flex flex-row justify-between">
                    <Link href="/" className="m-1 flex w-full justify-center rounded-xl border border-emerald-100 bg-emerald-100 p-2 text-emerald-500 hover:border-emerald-500">새로운 일기 작성하기</Link>
                    <button onClick={removeDiary} className=" m-1 flex w-full justify-center rounded-xl border border-red-100 bg-red-100 p-2 text-red-500 hover:border-red-500">현재 일기 삭제하기</button>
                </div>
